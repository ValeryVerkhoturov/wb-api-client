...
public class FullStatsItem {
  ...
  public FullStatsItem boosterStats(
      @jakarta.annotation.Nullable List<FullStatsItemBoosterStatsInner> boosterStats) {
  ...
  /**
   * Сумма заказов в базовых единицах валюты [аккаунта
   * продавца](https://cmp.wildberries.ru/campaigns/finances)
   *
   * @return sumPrice
   */
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!FullStatsItem.openapiRequiredFields
          .isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(
            String.format(
                "The required field(s) %s in FullStatsItem is not found in the empty JSON string",
                FullStatsItem.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!FullStatsItem.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(
            String.format(
                "The field `%s` in the JSON string is not defined in the `FullStatsItem` properties. JSON: %s",
    ...
    if (jsonObj.get("boosterStats") != null && !jsonObj.get("boosterStats").isJsonNull()) {
      ...
      if (jsonArrayboosterStats != null) {
        ...
        if (!jsonObj.get("boosterStats").isJsonArray()) {
          throw new IllegalArgumentException(
              String.format(
                  "Expected the field `boosterStats` to be an array in the JSON string but got `%s`",
                  jsonObj.get("boosterStats").toString()));
        }
