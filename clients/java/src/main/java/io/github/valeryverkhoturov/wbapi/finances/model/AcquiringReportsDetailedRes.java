/*
 * Документы и бухгалтерия
 * Узнать больше о документах и бухгалтерии можно в [справочном центре](https://seller.wildberries.ru/instructions/category/ba929b64-1f89-4426-82d7-ce998ee552bd?goBackOption=prevRoute&categoryId=3c971375-9939-45e8-ab82-376019be8942)  Просмотр [баланса](https://dev.wildberries.ru/openapi/documents-and-accounting#tag/balance), [финансовых отчётов](https://dev.wildberries.ru/openapi/documents-and-accounting#tag/financialReports) и [документов](https://dev.wildberries.ru/openapi/documents-and-accounting#tag/documents) продавца.
 *
 * The version of the OpenAPI document: finances
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */


package io.github.valeryverkhoturov.wbapi.finances.model;

import java.util.Objects;
import com.google.gson.TypeAdapter;
import com.google.gson.annotations.JsonAdapter;
import com.google.gson.annotations.SerializedName;
import com.google.gson.stream.JsonReader;
import com.google.gson.stream.JsonWriter;
import java.io.IOException;
import java.util.Arrays;

import com.google.gson.Gson;
import com.google.gson.GsonBuilder;
import com.google.gson.JsonArray;
import com.google.gson.JsonDeserializationContext;
import com.google.gson.JsonDeserializer;
import com.google.gson.JsonElement;
import com.google.gson.JsonObject;
import com.google.gson.JsonParseException;
import com.google.gson.TypeAdapterFactory;
import com.google.gson.reflect.TypeToken;
import com.google.gson.TypeAdapter;
import com.google.gson.stream.JsonReader;
import com.google.gson.stream.JsonWriter;
import java.io.IOException;

import java.util.HashMap;
import java.util.HashSet;
import java.util.List;
import java.util.Map;
import java.util.Set;

import io.github.valeryverkhoturov.wbapi.finances.JSON;

/**
 * Детализации к отчётам об издержках на приём платежей
 */
@jakarta.annotation.Generated(value = "org.openapitools.codegen.languages.JavaClientCodegen", comments = "Generator version: 7.10.0")
public class AcquiringReportsDetailedRes {
  public static final String SERIALIZED_NAME_RRD_ID = "rrdId";
  @SerializedName(SERIALIZED_NAME_RRD_ID)
  @jakarta.annotation.Nonnull
  private Integer rrdId;

  public static final String SERIALIZED_NAME_REPORT_ID = "reportId";
  @SerializedName(SERIALIZED_NAME_REPORT_ID)
  @jakarta.annotation.Nonnull
  private Long reportId;

  public static final String SERIALIZED_NAME_ACQ_DATE = "acqDate";
  @SerializedName(SERIALIZED_NAME_ACQ_DATE)
  @jakarta.annotation.Nonnull
  private String acqDate;

  public static final String SERIALIZED_NAME_ACQUIRING_BANK = "acquiringBank";
  @SerializedName(SERIALIZED_NAME_ACQUIRING_BANK)
  @jakarta.annotation.Nonnull
  private String acquiringBank;

  public static final String SERIALIZED_NAME_TIN = "tin";
  @SerializedName(SERIALIZED_NAME_TIN)
  @jakarta.annotation.Nonnull
  private String tin;

  public static final String SERIALIZED_NAME_TAX_REGISTRATION_REASON_CODE = "taxRegistrationReasonCode";
  @SerializedName(SERIALIZED_NAME_TAX_REGISTRATION_REASON_CODE)
  @jakarta.annotation.Nonnull
  private String taxRegistrationReasonCode;

  public static final String SERIALIZED_NAME_SALE_DATE = "saleDate";
  @SerializedName(SERIALIZED_NAME_SALE_DATE)
  @jakarta.annotation.Nonnull
  private String saleDate;

  public static final String SERIALIZED_NAME_SRID = "srid";
  @SerializedName(SERIALIZED_NAME_SRID)
  @jakarta.annotation.Nonnull
  private String srid;

  public static final String SERIALIZED_NAME_DOCUMENT_TYPE = "documentType";
  @SerializedName(SERIALIZED_NAME_DOCUMENT_TYPE)
  @jakarta.annotation.Nonnull
  private String documentType;

  public static final String SERIALIZED_NAME_NM_ID = "nmId";
  @SerializedName(SERIALIZED_NAME_NM_ID)
  @jakarta.annotation.Nonnull
  private Integer nmId;

  public static final String SERIALIZED_NAME_RETAIL_AMOUNT = "retailAmount";
  @SerializedName(SERIALIZED_NAME_RETAIL_AMOUNT)
  @jakarta.annotation.Nonnull
  private String retailAmount;

  public static final String SERIALIZED_NAME_ACQUIRING_FEE = "acquiringFee";
  @SerializedName(SERIALIZED_NAME_ACQUIRING_FEE)
  @jakarta.annotation.Nonnull
  private String acquiringFee;

  public static final String SERIALIZED_NAME_ACQUIRING_FEE_VAT = "acquiringFeeVat";
  @SerializedName(SERIALIZED_NAME_ACQUIRING_FEE_VAT)
  @jakarta.annotation.Nonnull
  private String acquiringFeeVat;

  public static final String SERIALIZED_NAME_INVOICE_NUMBER = "invoiceNumber";
  @SerializedName(SERIALIZED_NAME_INVOICE_NUMBER)
  @jakarta.annotation.Nonnull
  private String invoiceNumber;

  public static final String SERIALIZED_NAME_INVOICE_DATE = "invoiceDate";
  @SerializedName(SERIALIZED_NAME_INVOICE_DATE)
  @jakarta.annotation.Nonnull
  private String invoiceDate;

  public static final String SERIALIZED_NAME_SHK_ID = "shkId";
  @SerializedName(SERIALIZED_NAME_SHK_ID)
  @jakarta.annotation.Nonnull
  private Integer shkId;

  public static final String SERIALIZED_NAME_CURRENCY = "currency";
  @SerializedName(SERIALIZED_NAME_CURRENCY)
  @jakarta.annotation.Nonnull
  private String currency;

  public AcquiringReportsDetailedRes() {
  }

  public AcquiringReportsDetailedRes rrdId(@jakarta.annotation.Nonnull Integer rrdId) {
    this.rrdId = rrdId;
    return this;
  }

  /**
   * ID строки
   * @return rrdId
   */
  @jakarta.annotation.Nonnull
  public Integer getRrdId() {
    return rrdId;
  }

  public void setRrdId(@jakarta.annotation.Nonnull Integer rrdId) {
    this.rrdId = rrdId;
  }


  public AcquiringReportsDetailedRes reportId(@jakarta.annotation.Nonnull Long reportId) {
    this.reportId = reportId;
    return this;
  }

  /**
   * ID отчёта
   * @return reportId
   */
  @jakarta.annotation.Nonnull
  public Long getReportId() {
    return reportId;
  }

  public void setReportId(@jakarta.annotation.Nonnull Long reportId) {
    this.reportId = reportId;
  }


  public AcquiringReportsDetailedRes acqDate(@jakarta.annotation.Nonnull String acqDate) {
    this.acqDate = acqDate;
    return this;
  }

  /**
   * Дата операции
   * @return acqDate
   */
  @jakarta.annotation.Nonnull
  public String getAcqDate() {
    return acqDate;
  }

  public void setAcqDate(@jakarta.annotation.Nonnull String acqDate) {
    this.acqDate = acqDate;
  }


  public AcquiringReportsDetailedRes acquiringBank(@jakarta.annotation.Nonnull String acquiringBank) {
    this.acquiringBank = acquiringBank;
    return this;
  }

  /**
   * Наименование банка-эквайера
   * @return acquiringBank
   */
  @jakarta.annotation.Nonnull
  public String getAcquiringBank() {
    return acquiringBank;
  }

  public void setAcquiringBank(@jakarta.annotation.Nonnull String acquiringBank) {
    this.acquiringBank = acquiringBank;
  }


  public AcquiringReportsDetailedRes tin(@jakarta.annotation.Nonnull String tin) {
    this.tin = tin;
    return this;
  }

  /**
   * ИНН
   * @return tin
   */
  @jakarta.annotation.Nonnull
  public String getTin() {
    return tin;
  }

  public void setTin(@jakarta.annotation.Nonnull String tin) {
    this.tin = tin;
  }


  public AcquiringReportsDetailedRes taxRegistrationReasonCode(@jakarta.annotation.Nonnull String taxRegistrationReasonCode) {
    this.taxRegistrationReasonCode = taxRegistrationReasonCode;
    return this;
  }

  /**
   * КПП
   * @return taxRegistrationReasonCode
   */
  @jakarta.annotation.Nonnull
  public String getTaxRegistrationReasonCode() {
    return taxRegistrationReasonCode;
  }

  public void setTaxRegistrationReasonCode(@jakarta.annotation.Nonnull String taxRegistrationReasonCode) {
    this.taxRegistrationReasonCode = taxRegistrationReasonCode;
  }


  public AcquiringReportsDetailedRes saleDate(@jakarta.annotation.Nonnull String saleDate) {
    this.saleDate = saleDate;
    return this;
  }

  /**
   * Дата продажи
   * @return saleDate
   */
  @jakarta.annotation.Nonnull
  public String getSaleDate() {
    return saleDate;
  }

  public void setSaleDate(@jakarta.annotation.Nonnull String saleDate) {
    this.saleDate = saleDate;
  }


  public AcquiringReportsDetailedRes srid(@jakarta.annotation.Nonnull String srid) {
    this.srid = srid;
    return this;
  }

  /**
   * ID заказа. В ответах методов сборочных заданий [FBS](./orders-fbs#tag/fbsAssemblyOrders), [DBW](./orders-dbw#tag/dbwAssemblyOrders), [DBS](./dbs#tag/dbsAssemblyOrders) и [Самовывоз](./in-store-pickup#tag/inStorePickupAssemblyOrders) &#x60;srid&#x60; равен &#x60;rid&#x60;
   * @return srid
   */
  @jakarta.annotation.Nonnull
  public String getSrid() {
    return srid;
  }

  public void setSrid(@jakarta.annotation.Nonnull String srid) {
    this.srid = srid;
  }


  public AcquiringReportsDetailedRes documentType(@jakarta.annotation.Nonnull String documentType) {
    this.documentType = documentType;
    return this;
  }

  /**
   * Тип документа
   * @return documentType
   */
  @jakarta.annotation.Nonnull
  public String getDocumentType() {
    return documentType;
  }

  public void setDocumentType(@jakarta.annotation.Nonnull String documentType) {
    this.documentType = documentType;
  }


  public AcquiringReportsDetailedRes nmId(@jakarta.annotation.Nonnull Integer nmId) {
    this.nmId = nmId;
    return this;
  }

  /**
   * Артикул WB
   * @return nmId
   */
  @jakarta.annotation.Nonnull
  public Integer getNmId() {
    return nmId;
  }

  public void setNmId(@jakarta.annotation.Nonnull Integer nmId) {
    this.nmId = nmId;
  }


  public AcquiringReportsDetailedRes retailAmount(@jakarta.annotation.Nonnull String retailAmount) {
    this.retailAmount = retailAmount;
    return this;
  }

  /**
   * Wildberries реализовал Товар (Пр)
   * @return retailAmount
   */
  @jakarta.annotation.Nonnull
  public String getRetailAmount() {
    return retailAmount;
  }

  public void setRetailAmount(@jakarta.annotation.Nonnull String retailAmount) {
    this.retailAmount = retailAmount;
  }


  public AcquiringReportsDetailedRes acquiringFee(@jakarta.annotation.Nonnull String acquiringFee) {
    this.acquiringFee = acquiringFee;
    return this;
  }

  /**
   * Размер комиссии за эквайринг, в том числе НДС
   * @return acquiringFee
   */
  @jakarta.annotation.Nonnull
  public String getAcquiringFee() {
    return acquiringFee;
  }

  public void setAcquiringFee(@jakarta.annotation.Nonnull String acquiringFee) {
    this.acquiringFee = acquiringFee;
  }


  public AcquiringReportsDetailedRes acquiringFeeVat(@jakarta.annotation.Nonnull String acquiringFeeVat) {
    this.acquiringFeeVat = acquiringFeeVat;
    return this;
  }

  /**
   * Сумма НДС
   * @return acquiringFeeVat
   */
  @jakarta.annotation.Nonnull
  public String getAcquiringFeeVat() {
    return acquiringFeeVat;
  }

  public void setAcquiringFeeVat(@jakarta.annotation.Nonnull String acquiringFeeVat) {
    this.acquiringFeeVat = acquiringFeeVat;
  }


  public AcquiringReportsDetailedRes invoiceNumber(@jakarta.annotation.Nonnull String invoiceNumber) {
    this.invoiceNumber = invoiceNumber;
    return this;
  }

  /**
   * Номер счёта-фактуры
   * @return invoiceNumber
   */
  @jakarta.annotation.Nonnull
  public String getInvoiceNumber() {
    return invoiceNumber;
  }

  public void setInvoiceNumber(@jakarta.annotation.Nonnull String invoiceNumber) {
    this.invoiceNumber = invoiceNumber;
  }


  public AcquiringReportsDetailedRes invoiceDate(@jakarta.annotation.Nonnull String invoiceDate) {
    this.invoiceDate = invoiceDate;
    return this;
  }

  /**
   * Дата счёта-фактуры
   * @return invoiceDate
   */
  @jakarta.annotation.Nonnull
  public String getInvoiceDate() {
    return invoiceDate;
  }

  public void setInvoiceDate(@jakarta.annotation.Nonnull String invoiceDate) {
    this.invoiceDate = invoiceDate;
  }


  public AcquiringReportsDetailedRes shkId(@jakarta.annotation.Nonnull Integer shkId) {
    this.shkId = shkId;
    return this;
  }

  /**
   * Штрихкод
   * @return shkId
   */
  @jakarta.annotation.Nonnull
  public Integer getShkId() {
    return shkId;
  }

  public void setShkId(@jakarta.annotation.Nonnull Integer shkId) {
    this.shkId = shkId;
  }


  public AcquiringReportsDetailedRes currency(@jakarta.annotation.Nonnull String currency) {
    this.currency = currency;
    return this;
  }

  /**
   * Валюта отчёта
   * @return currency
   */
  @jakarta.annotation.Nonnull
  public String getCurrency() {
    return currency;
  }

  public void setCurrency(@jakarta.annotation.Nonnull String currency) {
    this.currency = currency;
  }



  @Override
  public boolean equals(Object o) {
    if (this == o) {
      return true;
    }
    if (o == null || getClass() != o.getClass()) {
      return false;
    }
    AcquiringReportsDetailedRes acquiringReportsDetailedRes = (AcquiringReportsDetailedRes) o;
    return Objects.equals(this.rrdId, acquiringReportsDetailedRes.rrdId) &&
        Objects.equals(this.reportId, acquiringReportsDetailedRes.reportId) &&
        Objects.equals(this.acqDate, acquiringReportsDetailedRes.acqDate) &&
        Objects.equals(this.acquiringBank, acquiringReportsDetailedRes.acquiringBank) &&
        Objects.equals(this.tin, acquiringReportsDetailedRes.tin) &&
        Objects.equals(this.taxRegistrationReasonCode, acquiringReportsDetailedRes.taxRegistrationReasonCode) &&
        Objects.equals(this.saleDate, acquiringReportsDetailedRes.saleDate) &&
        Objects.equals(this.srid, acquiringReportsDetailedRes.srid) &&
        Objects.equals(this.documentType, acquiringReportsDetailedRes.documentType) &&
        Objects.equals(this.nmId, acquiringReportsDetailedRes.nmId) &&
        Objects.equals(this.retailAmount, acquiringReportsDetailedRes.retailAmount) &&
        Objects.equals(this.acquiringFee, acquiringReportsDetailedRes.acquiringFee) &&
        Objects.equals(this.acquiringFeeVat, acquiringReportsDetailedRes.acquiringFeeVat) &&
        Objects.equals(this.invoiceNumber, acquiringReportsDetailedRes.invoiceNumber) &&
        Objects.equals(this.invoiceDate, acquiringReportsDetailedRes.invoiceDate) &&
        Objects.equals(this.shkId, acquiringReportsDetailedRes.shkId) &&
        Objects.equals(this.currency, acquiringReportsDetailedRes.currency);
  }

  @Override
  public int hashCode() {
    return Objects.hash(rrdId, reportId, acqDate, acquiringBank, tin, taxRegistrationReasonCode, saleDate, srid, documentType, nmId, retailAmount, acquiringFee, acquiringFeeVat, invoiceNumber, invoiceDate, shkId, currency);
  }

  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class AcquiringReportsDetailedRes {\n");
    sb.append("    rrdId: ").append(toIndentedString(rrdId)).append("\n");
    sb.append("    reportId: ").append(toIndentedString(reportId)).append("\n");
    sb.append("    acqDate: ").append(toIndentedString(acqDate)).append("\n");
    sb.append("    acquiringBank: ").append(toIndentedString(acquiringBank)).append("\n");
    sb.append("    tin: ").append(toIndentedString(tin)).append("\n");
    sb.append("    taxRegistrationReasonCode: ").append(toIndentedString(taxRegistrationReasonCode)).append("\n");
    sb.append("    saleDate: ").append(toIndentedString(saleDate)).append("\n");
    sb.append("    srid: ").append(toIndentedString(srid)).append("\n");
    sb.append("    documentType: ").append(toIndentedString(documentType)).append("\n");
    sb.append("    nmId: ").append(toIndentedString(nmId)).append("\n");
    sb.append("    retailAmount: ").append(toIndentedString(retailAmount)).append("\n");
    sb.append("    acquiringFee: ").append(toIndentedString(acquiringFee)).append("\n");
    sb.append("    acquiringFeeVat: ").append(toIndentedString(acquiringFeeVat)).append("\n");
    sb.append("    invoiceNumber: ").append(toIndentedString(invoiceNumber)).append("\n");
    sb.append("    invoiceDate: ").append(toIndentedString(invoiceDate)).append("\n");
    sb.append("    shkId: ").append(toIndentedString(shkId)).append("\n");
    sb.append("    currency: ").append(toIndentedString(currency)).append("\n");
    sb.append("}");
    return sb.toString();
  }

  /**
   * Convert the given object to string with each line indented by 4 spaces
   * (except the first line).
   */
  private String toIndentedString(Object o) {
    if (o == null) {
      return "null";
    }
    return o.toString().replace("\n", "\n    ");
  }


  public static HashSet<String> openapiFields;
  public static HashSet<String> openapiRequiredFields;

  static {
    // a set of all properties/fields (JSON key names)
    openapiFields = new HashSet<String>();
    openapiFields.add("rrdId");
    openapiFields.add("reportId");
    openapiFields.add("acqDate");
    openapiFields.add("acquiringBank");
    openapiFields.add("tin");
    openapiFields.add("taxRegistrationReasonCode");
    openapiFields.add("saleDate");
    openapiFields.add("srid");
    openapiFields.add("documentType");
    openapiFields.add("nmId");
    openapiFields.add("retailAmount");
    openapiFields.add("acquiringFee");
    openapiFields.add("acquiringFeeVat");
    openapiFields.add("invoiceNumber");
    openapiFields.add("invoiceDate");
    openapiFields.add("shkId");
    openapiFields.add("currency");

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>();
    openapiRequiredFields.add("rrdId");
    openapiRequiredFields.add("reportId");
    openapiRequiredFields.add("acqDate");
    openapiRequiredFields.add("acquiringBank");
    openapiRequiredFields.add("tin");
    openapiRequiredFields.add("taxRegistrationReasonCode");
    openapiRequiredFields.add("saleDate");
    openapiRequiredFields.add("srid");
    openapiRequiredFields.add("documentType");
    openapiRequiredFields.add("nmId");
    openapiRequiredFields.add("retailAmount");
    openapiRequiredFields.add("acquiringFee");
    openapiRequiredFields.add("acquiringFeeVat");
    openapiRequiredFields.add("invoiceNumber");
    openapiRequiredFields.add("invoiceDate");
    openapiRequiredFields.add("shkId");
    openapiRequiredFields.add("currency");
  }

  /**
   * Validates the JSON Element and throws an exception if issues found
   *
   * @param jsonElement JSON Element
   * @throws IOException if the JSON Element is invalid with respect to AcquiringReportsDetailedRes
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AcquiringReportsDetailedRes.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in AcquiringReportsDetailedRes is not found in the empty JSON string", AcquiringReportsDetailedRes.openapiRequiredFields.toString()));
        }
      }

      Set<Map.Entry<String, JsonElement>> entries = jsonElement.getAsJsonObject().entrySet();
      // check to see if the JSON string contains additional fields
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!AcquiringReportsDetailedRes.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AcquiringReportsDetailedRes` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : AcquiringReportsDetailedRes.openapiRequiredFields) {
        if (jsonElement.getAsJsonObject().get(requiredField) == null) {
          throw new IllegalArgumentException(String.format("The required field `%s` is not found in the JSON string: %s", requiredField, jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      if (!jsonObj.get("acqDate").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format("Expected the field `acqDate` to be a primitive type in the JSON string but got `%s`", jsonObj.get("acqDate").toString()));
      }
      if (!jsonObj.get("acquiringBank").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format("Expected the field `acquiringBank` to be a primitive type in the JSON string but got `%s`", jsonObj.get("acquiringBank").toString()));
      }
      if (!jsonObj.get("tin").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format("Expected the field `tin` to be a primitive type in the JSON string but got `%s`", jsonObj.get("tin").toString()));
      }
      if (!jsonObj.get("taxRegistrationReasonCode").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format("Expected the field `taxRegistrationReasonCode` to be a primitive type in the JSON string but got `%s`", jsonObj.get("taxRegistrationReasonCode").toString()));
      }
      if (!jsonObj.get("saleDate").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format("Expected the field `saleDate` to be a primitive type in the JSON string but got `%s`", jsonObj.get("saleDate").toString()));
      }
      if (!jsonObj.get("srid").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format("Expected the field `srid` to be a primitive type in the JSON string but got `%s`", jsonObj.get("srid").toString()));
      }
      if (!jsonObj.get("documentType").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format("Expected the field `documentType` to be a primitive type in the JSON string but got `%s`", jsonObj.get("documentType").toString()));
      }
      if (!jsonObj.get("retailAmount").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format("Expected the field `retailAmount` to be a primitive type in the JSON string but got `%s`", jsonObj.get("retailAmount").toString()));
      }
      if (!jsonObj.get("acquiringFee").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format("Expected the field `acquiringFee` to be a primitive type in the JSON string but got `%s`", jsonObj.get("acquiringFee").toString()));
      }
      if (!jsonObj.get("acquiringFeeVat").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format("Expected the field `acquiringFeeVat` to be a primitive type in the JSON string but got `%s`", jsonObj.get("acquiringFeeVat").toString()));
      }
      if (!jsonObj.get("invoiceNumber").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format("Expected the field `invoiceNumber` to be a primitive type in the JSON string but got `%s`", jsonObj.get("invoiceNumber").toString()));
      }
      if (!jsonObj.get("invoiceDate").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format("Expected the field `invoiceDate` to be a primitive type in the JSON string but got `%s`", jsonObj.get("invoiceDate").toString()));
      }
      if (!jsonObj.get("currency").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format("Expected the field `currency` to be a primitive type in the JSON string but got `%s`", jsonObj.get("currency").toString()));
      }
  }

  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    @SuppressWarnings("unchecked")
    @Override
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!AcquiringReportsDetailedRes.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'AcquiringReportsDetailedRes' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<AcquiringReportsDetailedRes> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(AcquiringReportsDetailedRes.class));

       return (TypeAdapter<T>) new TypeAdapter<AcquiringReportsDetailedRes>() {
           @Override
           public void write(JsonWriter out, AcquiringReportsDetailedRes value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public AcquiringReportsDetailedRes read(JsonReader in) throws IOException {
             JsonElement jsonElement = elementAdapter.read(in);
             validateJsonElement(jsonElement);
             return thisAdapter.fromJsonTree(jsonElement);
           }

       }.nullSafe();
    }
  }

  /**
   * Create an instance of AcquiringReportsDetailedRes given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of AcquiringReportsDetailedRes
   * @throws IOException if the JSON string is invalid with respect to AcquiringReportsDetailedRes
   */
  public static AcquiringReportsDetailedRes fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, AcquiringReportsDetailedRes.class);
  }

  /**
   * Convert an instance of AcquiringReportsDetailedRes to an JSON string
   *
   * @return JSON string
   */
  public String toJson() {
    return JSON.getGson().toJson(this);
  }
}

