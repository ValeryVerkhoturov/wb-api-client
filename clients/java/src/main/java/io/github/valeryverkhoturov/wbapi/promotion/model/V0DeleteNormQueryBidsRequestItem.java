...
public class V0DeleteNormQueryBidsRequestItem {
  ...
  public boolean equals(Object o) {
    ...
    V0DeleteNormQueryBidsRequestItem v0DeleteNormQueryBidsRequestItem =
        (V0DeleteNormQueryBidsRequestItem) o;
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!V0DeleteNormQueryBidsRequestItem.openapiRequiredFields
          .isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(
            String.format(
                "The required field(s) %s in V0DeleteNormQueryBidsRequestItem is not found in the empty JSON string",
                V0DeleteNormQueryBidsRequestItem.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!V0DeleteNormQueryBidsRequestItem.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(
            String.format(
                "The field `%s` in the JSON string is not defined in the `V0DeleteNormQueryBidsRequestItem` properties. JSON: %s",
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      if (!V0DeleteNormQueryBidsRequestItem.class.isAssignableFrom(type.getRawType())) {
        return null; // this class only serializes 'V0DeleteNormQueryBidsRequestItem' and its
        // subtypes
      }
      ...
      return (TypeAdapter<T>)
          new TypeAdapter<V0DeleteNormQueryBidsRequestItem>() {
            @Override
            public void write(JsonWriter out, V0DeleteNormQueryBidsRequestItem value)
                throws IOException {
