...
public class Office {
  ...
  /**
   * Тип товара, который принимает склад: - &#x60;1&#x60; — малогабаритный товар (МГТ) -
  ...
  /**
   * Федеральный округ склада WB. Если &#x60;null&#x60;, склад находится за пределами РФ или
   * федеральный округ не указан
   *
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!Office.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(
            String.format(
                "The required field(s) %s in Office is not found in the empty JSON string",
                Office.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!Office.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(
            String.format(
                "The field `%s` in the JSON string is not defined in the `Office` properties. JSON: %s",
    ...
    if ((jsonObj.get("federalDistrict") != null && !jsonObj.get("federalDistrict").isJsonNull())
        && !jsonObj.get("federalDistrict").isJsonPrimitive()) {
      throw new IllegalArgumentException(
          String.format(
              "Expected the field `federalDistrict` to be a primitive type in the JSON string but got `%s`",
              jsonObj.get("federalDistrict").toString()));
    }
  }
