...
import io.github.valeryverkhoturov.wbapi.dbs.model.OrderNewDBSAddress;
import io.github.valeryverkhoturov.wbapi.dbs.model.OrderNewDBSOptions;
...
public class OrderNewDBS {
  ...
  public OrderNewDBS convertedCurrencyCode(@jakarta.annotation.Nullable Integer convertedCurrencyCode) {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(salePrice, requiredMeta, comment, options, address, orderUid, groupId, article, colorCode, rid, createdAt, deliveryType, skus, id, warehouseId, nmId, chrtId, price, finalPrice, convertedFinalPrice, convertedPrice, currencyCode, convertedCurrencyCode, cargoType, isZeroOrder, wbStickerId);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!OrderNewDBS.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in OrderNewDBS is not found in the empty JSON string", OrderNewDBS.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!OrderNewDBS.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `OrderNewDBS` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       ...
       return (TypeAdapter<T>) new TypeAdapter<OrderNewDBS>() {
           @Override
