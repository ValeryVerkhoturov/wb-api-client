...
public class ApiErrorV3 {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ApiErrorV3
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ApiErrorV3.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ApiErrorV3 is not found in the empty JSON string", ApiErrorV3.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ApiErrorV3.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ApiErrorV3` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ApiErrorV3.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ApiErrorV3' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ApiErrorV3> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ApiErrorV3.class));

       return (TypeAdapter<T>) new TypeAdapter<ApiErrorV3>() {
           @Override
           public void write(JsonWriter out, ApiErrorV3 value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ApiErrorV3 read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of ApiErrorV3 given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of ApiErrorV3
   * @throws IOException if the JSON string is invalid with respect to ApiErrorV3
   */
  public static ApiErrorV3 fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ApiErrorV3.class);
  }

  /**
   * Convert an instance of ApiErrorV3 to an JSON string
   *
   * @return JSON string
   */
