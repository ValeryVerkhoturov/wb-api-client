/*
 * Отчёты
 * Узнать больше об отчётах можно в [справочном центре](https://seller.wildberries.ru/instructions/subcategory/5f2162c5-069b-416d-a4e1-48da2a76e6b0)  С помощью этих методов вы можете получать [основные отчёты](https://dev.wildberries.ru/openapi/reports#tag/mainReports) и отчёты о: 1. [Остатках на складах](https://dev.wildberries.ru/openapi/reports#tag/warehousesInventoryReport) 2. [Товарах с обязательной маркировкой](https://dev.wildberries.ru/openapi/reports#tag/reportOnItemsWithMandatoryLabeling) 3. [Удержаниях](https://dev.wildberries.ru/openapi/reports#tag/retentionReports) 4. [Операциях при приёмке](https://dev.wildberries.ru/openapi/reports#tag/acceptanceExpenses) 5. [Платном хранении](https://dev.wildberries.ru/openapi/reports#tag/paidStorage) 6. [Продажах по регионам](https://dev.wildberries.ru/openapi/reports#tag/salesByRegions) 7. [Доле бренда в продажах](https://dev.wildberries.ru/openapi/reports#tag/shareOfBrandInSales) 8. [Заблокированных карточках](https://dev.wildberries.ru/openapi/reports#tag/blockedItems) 9. [Возвратах и перемещении товаров](https://dev.wildberries.ru/openapi/reports#tag/returnsAndItemMovementReport)
 *
 * The version of the OpenAPI document: reports
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

package io.github.valeryverkhoturov.wbapi.reports.model;

import com.google.gson.Gson;
import com.google.gson.JsonArray;
import com.google.gson.JsonElement;
import com.google.gson.JsonObject;
import com.google.gson.TypeAdapter;
import com.google.gson.TypeAdapterFactory;
import com.google.gson.annotations.SerializedName;
import com.google.gson.reflect.TypeToken;
import com.google.gson.stream.JsonReader;
import com.google.gson.stream.JsonWriter;
import io.github.valeryverkhoturov.wbapi.reports.JSON;
import java.io.IOException;
import java.math.BigDecimal;
import java.util.ArrayList;
import java.util.HashSet;
import java.util.List;
import java.util.Map;
import java.util.Objects;
import java.util.Set;

/** GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner */
@jakarta.annotation.Generated(
    value = "org.openapitools.codegen.languages.JavaClientCodegen",
    comments = "Generator version: 7.10.0")
public class GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner {
  public static final String SERIALIZED_NAME_BRAND = "brand";

  @SerializedName(SERIALIZED_NAME_BRAND)
  @jakarta.annotation.Nullable
  private String brand;

  public static final String SERIALIZED_NAME_SUBJECT_NAME = "subjectName";

  @SerializedName(SERIALIZED_NAME_SUBJECT_NAME)
  @jakarta.annotation.Nullable
  private String subjectName;

  public static final String SERIALIZED_NAME_VENDOR_CODE = "vendorCode";

  @SerializedName(SERIALIZED_NAME_VENDOR_CODE)
  @jakarta.annotation.Nullable
  private String vendorCode;

  public static final String SERIALIZED_NAME_NM_ID = "nmId";

  @SerializedName(SERIALIZED_NAME_NM_ID)
  @jakarta.annotation.Nullable
  private Integer nmId;

  public static final String SERIALIZED_NAME_BARCODE = "barcode";

  @SerializedName(SERIALIZED_NAME_BARCODE)
  @jakarta.annotation.Nullable
  private String barcode;

  public static final String SERIALIZED_NAME_TECH_SIZE = "techSize";

  @SerializedName(SERIALIZED_NAME_TECH_SIZE)
  @jakarta.annotation.Nullable
  private String techSize;

  public static final String SERIALIZED_NAME_VOLUME = "volume";

  @SerializedName(SERIALIZED_NAME_VOLUME)
  @jakarta.annotation.Nullable
  private BigDecimal volume;

  public static final String SERIALIZED_NAME_WAREHOUSES = "warehouses";

  @SerializedName(SERIALIZED_NAME_WAREHOUSES)
  @jakarta.annotation.Nullable
  private List<GetV1WarehouseRemainsTasksTaskIdDownloadResponse200InnerWarehousesInner> warehouses =
      new ArrayList<>();

  public GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner() {}

  public GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner brand(
      @jakarta.annotation.Nullable String brand) {
    this.brand = brand;
    return this;
  }

  /**
   * Бренд
   *
   * @return brand
   */
  @jakarta.annotation.Nullable
  public String getBrand() {
    return brand;
  }

  public void setBrand(@jakarta.annotation.Nullable String brand) {
    this.brand = brand;
  }

  public GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner subjectName(
      @jakarta.annotation.Nullable String subjectName) {
    this.subjectName = subjectName;
    return this;
  }

  /**
   * Название предмета
   *
   * @return subjectName
   */
  @jakarta.annotation.Nullable
  public String getSubjectName() {
    return subjectName;
  }

  public void setSubjectName(@jakarta.annotation.Nullable String subjectName) {
    this.subjectName = subjectName;
  }

  public GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner vendorCode(
      @jakarta.annotation.Nullable String vendorCode) {
    this.vendorCode = vendorCode;
    return this;
  }

  /**
   * Артикул продавца
   *
   * @return vendorCode
   */
  @jakarta.annotation.Nullable
  public String getVendorCode() {
    return vendorCode;
  }

  public void setVendorCode(@jakarta.annotation.Nullable String vendorCode) {
    this.vendorCode = vendorCode;
  }

  public GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner nmId(
      @jakarta.annotation.Nullable Integer nmId) {
    this.nmId = nmId;
    return this;
  }

  /**
   * Артикул WB
   *
   * @return nmId
   */
  @jakarta.annotation.Nullable
  public Integer getNmId() {
    return nmId;
  }

  public void setNmId(@jakarta.annotation.Nullable Integer nmId) {
    this.nmId = nmId;
  }

  public GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner barcode(
      @jakarta.annotation.Nullable String barcode) {
    this.barcode = barcode;
    return this;
  }

  /**
   * Баркод
   *
   * @return barcode
   */
  @jakarta.annotation.Nullable
  public String getBarcode() {
    return barcode;
  }

  public void setBarcode(@jakarta.annotation.Nullable String barcode) {
    this.barcode = barcode;
  }

  public GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner techSize(
      @jakarta.annotation.Nullable String techSize) {
    this.techSize = techSize;
    return this;
  }

  /**
   * Размер
   *
   * @return techSize
   */
  @jakarta.annotation.Nullable
  public String getTechSize() {
    return techSize;
  }

  public void setTechSize(@jakarta.annotation.Nullable String techSize) {
    this.techSize = techSize;
  }

  public GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner volume(
      @jakarta.annotation.Nullable BigDecimal volume) {
    this.volume = volume;
    return this;
  }

  /**
   * Объём, л
   *
   * @return volume
   */
  @jakarta.annotation.Nullable
  public BigDecimal getVolume() {
    return volume;
  }

  public void setVolume(@jakarta.annotation.Nullable BigDecimal volume) {
    this.volume = volume;
  }

  public GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner warehouses(
      @jakarta.annotation.Nullable
          List<GetV1WarehouseRemainsTasksTaskIdDownloadResponse200InnerWarehousesInner>
              warehouses) {
    this.warehouses = warehouses;
    return this;
  }

  public GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner addWarehousesItem(
      GetV1WarehouseRemainsTasksTaskIdDownloadResponse200InnerWarehousesInner warehousesItem) {
    if (this.warehouses == null) {
      this.warehouses = new ArrayList<>();
    }
    this.warehouses.add(warehousesItem);
    return this;
  }

  /**
   * Остатки на складах и товары в пути. Будут в ответе только при ненулевом &#x60;quantity&#x60;
   *
   * @return warehouses
   */
  @jakarta.annotation.Nullable
  public List<GetV1WarehouseRemainsTasksTaskIdDownloadResponse200InnerWarehousesInner>
      getWarehouses() {
    return warehouses;
  }

  public void setWarehouses(
      @jakarta.annotation.Nullable
          List<GetV1WarehouseRemainsTasksTaskIdDownloadResponse200InnerWarehousesInner>
              warehouses) {
    this.warehouses = warehouses;
  }

  @Override
  public boolean equals(Object o) {
    if (this == o) {
      return true;
    }
    if (o == null || getClass() != o.getClass()) {
      return false;
    }
    GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner
        getV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner =
            (GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner) o;
    return Objects.equals(
            this.brand, getV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner.brand)
        && Objects.equals(
            this.subjectName, getV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner.subjectName)
        && Objects.equals(
            this.vendorCode, getV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner.vendorCode)
        && Objects.equals(this.nmId, getV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner.nmId)
        && Objects.equals(
            this.barcode, getV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner.barcode)
        && Objects.equals(
            this.techSize, getV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner.techSize)
        && Objects.equals(
            this.volume, getV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner.volume)
        && Objects.equals(
            this.warehouses, getV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner.warehouses);
  }

  @Override
  public int hashCode() {
    return Objects.hash(
        brand, subjectName, vendorCode, nmId, barcode, techSize, volume, warehouses);
  }

  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner {\n");
    sb.append("    brand: ").append(toIndentedString(brand)).append("\n");
    sb.append("    subjectName: ").append(toIndentedString(subjectName)).append("\n");
    sb.append("    vendorCode: ").append(toIndentedString(vendorCode)).append("\n");
    sb.append("    nmId: ").append(toIndentedString(nmId)).append("\n");
    sb.append("    barcode: ").append(toIndentedString(barcode)).append("\n");
    sb.append("    techSize: ").append(toIndentedString(techSize)).append("\n");
    sb.append("    volume: ").append(toIndentedString(volume)).append("\n");
    sb.append("    warehouses: ").append(toIndentedString(warehouses)).append("\n");
    sb.append("}");
    return sb.toString();
  }

  /**
   * Convert the given object to string with each line indented by 4 spaces (except the first line).
   */
  private String toIndentedString(Object o) {
    if (o == null) {
      return "null";
    }
    return o.toString().replace("\n", "\n    ");
  }

  public static HashSet<String> openapiFields;
  public static HashSet<String> openapiRequiredFields;

  static {
    // a set of all properties/fields (JSON key names)
    openapiFields = new HashSet<String>();
    openapiFields.add("brand");
    openapiFields.add("subjectName");
    openapiFields.add("vendorCode");
    openapiFields.add("nmId");
    openapiFields.add("barcode");
    openapiFields.add("techSize");
    openapiFields.add("volume");
    openapiFields.add("warehouses");

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>();
  }

  /**
   * Validates the JSON Element and throws an exception if issues found
   *
   * @param jsonElement JSON Element
   * @throws IOException if the JSON Element is invalid with respect to
   *     GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner.openapiRequiredFields
          .isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(
            String.format(
                "The required field(s) %s in GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner is not found in the empty JSON string",
                GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner.openapiRequiredFields
                    .toString()));
      }
    }

    Set<Map.Entry<String, JsonElement>> entries = jsonElement.getAsJsonObject().entrySet();
    // check to see if the JSON string contains additional fields
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner.openapiFields.contains(
          entry.getKey())) {
        throw new IllegalArgumentException(
            String.format(
                "The field `%s` in the JSON string is not defined in the `GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner` properties. JSON: %s",
                entry.getKey(), jsonElement.toString()));
      }
    }
    JsonObject jsonObj = jsonElement.getAsJsonObject();
    if ((jsonObj.get("brand") != null && !jsonObj.get("brand").isJsonNull())
        && !jsonObj.get("brand").isJsonPrimitive()) {
      throw new IllegalArgumentException(
          String.format(
              "Expected the field `brand` to be a primitive type in the JSON string but got `%s`",
              jsonObj.get("brand").toString()));
    }
    if ((jsonObj.get("subjectName") != null && !jsonObj.get("subjectName").isJsonNull())
        && !jsonObj.get("subjectName").isJsonPrimitive()) {
      throw new IllegalArgumentException(
          String.format(
              "Expected the field `subjectName` to be a primitive type in the JSON string but got `%s`",
              jsonObj.get("subjectName").toString()));
    }
    if ((jsonObj.get("vendorCode") != null && !jsonObj.get("vendorCode").isJsonNull())
        && !jsonObj.get("vendorCode").isJsonPrimitive()) {
      throw new IllegalArgumentException(
          String.format(
              "Expected the field `vendorCode` to be a primitive type in the JSON string but got `%s`",
              jsonObj.get("vendorCode").toString()));
    }
    if ((jsonObj.get("barcode") != null && !jsonObj.get("barcode").isJsonNull())
        && !jsonObj.get("barcode").isJsonPrimitive()) {
      throw new IllegalArgumentException(
          String.format(
              "Expected the field `barcode` to be a primitive type in the JSON string but got `%s`",
              jsonObj.get("barcode").toString()));
    }
    if ((jsonObj.get("techSize") != null && !jsonObj.get("techSize").isJsonNull())
        && !jsonObj.get("techSize").isJsonPrimitive()) {
      throw new IllegalArgumentException(
          String.format(
              "Expected the field `techSize` to be a primitive type in the JSON string but got `%s`",
              jsonObj.get("techSize").toString()));
    }
    if (jsonObj.get("warehouses") != null && !jsonObj.get("warehouses").isJsonNull()) {
      JsonArray jsonArraywarehouses = jsonObj.getAsJsonArray("warehouses");
      if (jsonArraywarehouses != null) {
        // ensure the json data is an array
        if (!jsonObj.get("warehouses").isJsonArray()) {
          throw new IllegalArgumentException(
              String.format(
                  "Expected the field `warehouses` to be an array in the JSON string but got `%s`",
                  jsonObj.get("warehouses").toString()));
        }

        // validate the optional field `warehouses` (array)
        for (int i = 0; i < jsonArraywarehouses.size(); i++) {
          GetV1WarehouseRemainsTasksTaskIdDownloadResponse200InnerWarehousesInner
              .validateJsonElement(jsonArraywarehouses.get(i));
        }
        ;
      }
    }
  }

  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    @SuppressWarnings("unchecked")
    @Override
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      if (!GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner.class.isAssignableFrom(
          type.getRawType())) {
        return null; // this class only serializes
        // 'GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner' and its subtypes
      }
      final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
      final TypeAdapter<GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner> thisAdapter =
          gson.getDelegateAdapter(
              this, TypeToken.get(GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner.class));

      return (TypeAdapter<T>)
          new TypeAdapter<GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner>() {
            @Override
            public void write(
                JsonWriter out, GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner value)
                throws IOException {
              JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
              elementAdapter.write(out, obj);
            }

            @Override
            public GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner read(JsonReader in)
                throws IOException {
              JsonElement jsonElement = elementAdapter.read(in);
              validateJsonElement(jsonElement);
              return thisAdapter.fromJsonTree(jsonElement);
            }
          }.nullSafe();
    }
  }

  /**
   * Create an instance of GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner given an JSON
   * string
   *
   * @param jsonString JSON string
   * @return An instance of GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner
   * @throws IOException if the JSON string is invalid with respect to
   *     GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner
   */
  public static GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner fromJson(String jsonString)
      throws IOException {
    return JSON.getGson()
        .fromJson(jsonString, GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner.class);
  }

  /**
   * Convert an instance of GetV1WarehouseRemainsTasksTaskIdDownloadResponse200Inner to an JSON
   * string
   *
   * @return JSON string
   */
  public String toJson() {
    return JSON.getGson().toJson(this);
  }
}
