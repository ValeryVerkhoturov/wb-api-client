...
public class Listing {
  ...
  /**
   * Размер товара, соответствует &#x60;wbSize&#x60; в [карточке
   * товара](./item-management#tag/listings/operation/postV2GetCardsList)
   *
   * @return size
   */
  @jakarta.annotation.Nullable
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!Listing.openapiRequiredFields
          .isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(
            String.format(
                "The required field(s) %s in Listing is not found in the empty JSON string",
                Listing.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!Listing.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(
            String.format(
                "The field `%s` in the JSON string is not defined in the `Listing` properties. JSON: %s",
                entry.getKey(), jsonElement.toString()));
      }
    }
    JsonObject jsonObj = jsonElement.getAsJsonObject();
    if ((jsonObj.get("priceCurrency") != null && !jsonObj.get("priceCurrency").isJsonNull())
        && !jsonObj.get("priceCurrency").isJsonPrimitive()) {
      throw new IllegalArgumentException(
          String.format(
              "Expected the field `priceCurrency` to be a primitive type in the JSON string but got `%s`",
              jsonObj.get("priceCurrency").toString()));
    }
