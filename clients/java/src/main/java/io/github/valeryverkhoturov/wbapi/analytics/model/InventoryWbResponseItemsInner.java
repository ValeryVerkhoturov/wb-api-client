...
public class InventoryWbResponseItemsInner {
  ...
  /**
   * Регион отгрузки. [На данный момент](https://dev.wildberries.ru/release-notes?id&#x3D;570) может
   * быть только &#x60;Склад WB&#x60;
   *
   * @return regionName
   */
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!InventoryWbResponseItemsInner.openapiRequiredFields
          .isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(
            String.format(
                "The required field(s) %s in InventoryWbResponseItemsInner is not found in the empty JSON string",
                InventoryWbResponseItemsInner.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!InventoryWbResponseItemsInner.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(
            String.format(
                "The field `%s` in the JSON string is not defined in the `InventoryWbResponseItemsInner` properties. JSON: %s",
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      return (TypeAdapter<T>)
          new TypeAdapter<InventoryWbResponseItemsInner>() {
            @Override
            public void write(JsonWriter out, InventoryWbResponseItemsInner value)
                throws IOException {
