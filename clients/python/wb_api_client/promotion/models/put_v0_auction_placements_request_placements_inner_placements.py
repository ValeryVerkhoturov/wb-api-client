...
class PutV0AuctionPlacementsRequestPlacementsInnerPlacements(BaseModel):
    ...
    search: StrictBool = Field(
        description="Размещение в поиске:   - `false` — отключено   - `true` — включено "
    )
    recommendations: StrictBool = Field(
        description="Размещение в рекомендациях:   - `false` — отключено   - `true` — включено "
    )
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {"search": obj.get("search"), "recommendations": obj.get("recommendations")}
        )
        return _obj
