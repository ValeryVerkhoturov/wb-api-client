...
class ModelsSupply(BaseModel):
    ...
    supply_id: Optional[StrictInt] = Field(
        default=None,
        description="ID поставки. Если `null`, это заказ, тогда используйте значение поля `preorderID`",
        alias="supplyID",
    )
    preorder_id: Optional[StrictInt] = Field(
        default=None,
        description="ID заказа (незапланированная поставка). Для всех виртуальных поставок будет `0`",
        alias="preorderID",
    )
    ...
    box_type_id: Optional[Any] = Field(
        default=None,
