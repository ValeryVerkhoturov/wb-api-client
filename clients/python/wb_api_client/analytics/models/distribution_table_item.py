...
class DistributionTableItem(BaseModel):
    ...
    pinned_feedback: StrictBool = Field(description="Отзыв закреплён", alias="pinnedFeedback")
    ...
    is_shadowed: StrictBool = Field(description="Является ли товар скрытым из каталога:   - `true` — товар скрыт из каталога   - `false` — товар не скрыт из каталога ", alias="isShadowed")
