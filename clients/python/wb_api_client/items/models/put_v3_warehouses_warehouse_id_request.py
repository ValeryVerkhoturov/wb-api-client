...
class PutV3WarehousesWarehouseIdRequest(BaseModel):
    ...
    office_id: Annotated[int, Field(strict=True, ge=1)] = Field(description="ID [склада WB](./item-management#tag/sellerWarehouses/operation/getV3Offices). Нельзя привязывать склад WB, который уже используется. Можно менять не чаще одного раза в сутки", alias="officeId")
