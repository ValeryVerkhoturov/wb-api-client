...
class PostV2CardsUploadAddRequestCardsToAddInner(BaseModel):
    ...
    sizes: Optional[List[PostV2CardsUploadAddRequestCardsToAddInnerSizesInner]] = Field(default=None, description="Массив размеров. Если не указать для размерного товара (обувь, одежда и др.), сгенерируется автоматически с `techSize` = \"A\", `wbSize` = \"1\" и баркодом")
