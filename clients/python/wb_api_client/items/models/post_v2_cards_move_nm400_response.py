...
POSTV2CARDSMOVENM400RESPONSE_ONE_OF_SCHEMAS = [
...
class PostV2CardsMoveNm400Response(BaseModel):
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, ResponseItemList):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `ResponseItemList`"
            )
        else:
            match += 1
        ...
        if not isinstance(v, ResponseIncorrectDate):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `ResponseIncorrectDate`"
            )
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in PostV2CardsMoveNm400Response with oneOf schemas: ResponseIncorrectDate, ResponseItemList. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in PostV2CardsMoveNm400Response with oneOf schemas: ResponseIncorrectDate, ResponseItemList. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when deserializing the JSON string into PostV2CardsMoveNm400Response with oneOf schemas: ResponseIncorrectDate, ResponseItemList. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into PostV2CardsMoveNm400Response with oneOf schemas: ResponseIncorrectDate, ResponseItemList. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(
        self,
    ) -> Optional[Union[Dict[str, Any], ResponseIncorrectDate, ResponseItemList]]:
