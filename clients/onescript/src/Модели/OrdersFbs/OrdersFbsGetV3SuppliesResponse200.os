// OrdersFbsGetV3SuppliesResponse200
//
...
// supplies - Массив - Список поставок
Перем supplies Экспорт;
...
Процедура Заполнить(Знач Данные) Экспорт
	...
	КонецЕсли;
	Если Значения.Получить("supplies") <> Неопределено Тогда
		supplies = Значения.Получить("supplies");
	КонецЕсли;
КонецПроцедуры
...
Функция Данные() Экспорт
	...
	КонецЕсли;
	Если supplies <> Неопределено Тогда
		Результат.Вставить("supplies", ЗначениеДляJSON(supplies));
	КонецЕсли;
	Возврат Результат;
