// OrdersFbsV3ArchiveOrderCrossBorder
//
...
// parcel - Строка - ID посылки
Перем parcel Экспорт;
...
Процедура Заполнить(Знач Данные) Экспорт
	...
	КонецЦикла;
	Если Значения.Получить("parcel") <> Неопределено Тогда
		parcel = Значения.Получить("parcel");
	КонецЕсли;
КонецПроцедуры
...
Функция Данные() Экспорт
	...
	Если parcel <> Неопределено Тогда
		Результат.Вставить("parcel", ЗначениеДляJSON(parcel));
	КонецЕсли;
	Возврат Результат;
