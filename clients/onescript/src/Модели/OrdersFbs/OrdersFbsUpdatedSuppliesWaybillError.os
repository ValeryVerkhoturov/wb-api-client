// OrdersFbsUpdatedSuppliesWaybillError
//
// Ошибка обработки запроса для поставки. Возможные варианты ошибок:
// - `400 IncorrectRequestBody`:
// - некорректный ID поставки
// - склад назначения находится не в РФ
// - `404 NotFound` — поставка не найдена
// - `409 SupplyAlreadyScanned` — поставка или её короба уже отсканированы в пункте отгрузки
// - `409 SupplyShippingRequired` — не указан пункт отгрузки поставки
// - `409 UnsuitableShippingType` — не указан способ доставки либо способ доставки не `transportCompany`
// - `409 WaybillUUIDIsProcessing` — ЭТрН находится в обработке
//
//
