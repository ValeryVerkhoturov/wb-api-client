// OrdersFbsPostV3SuppliesSupplyIdTrbxStickersRequest
//
// Класс сформирован автоматически (onescript) из спецификации OpenAPI.
// Правки будут потеряны при следующей генерации.

// trbxIds - Массив - Список ID грузомест, по которым необходимо вернуть стикеры
Перем trbxIds Экспорт;
...
Процедура Заполнить(Знач Данные) Экспорт
	...
	КонецЦикла;
	Если Значения.Получить("trbxIds") <> Неопределено Тогда
		trbxIds = Значения.Получить("trbxIds");
	КонецЕсли;
КонецПроцедуры
...
Функция Данные() Экспорт
	...
	Если trbxIds <> Неопределено Тогда
		Результат.Вставить("trbxIds", ЗначениеДляJSON(trbxIds));
	КонецЕсли;
	Возврат Результат;
