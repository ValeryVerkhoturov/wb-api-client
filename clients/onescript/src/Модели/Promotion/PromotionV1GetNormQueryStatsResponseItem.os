// PromotionV1GetNormQueryStatsResponseItem
//
...
// dailyStats - Массив - Статистика с детализацией по дням
Перем dailyStats Экспорт;
...
Процедура Заполнить(Знач Данные) Экспорт
	...
	КонецЕсли;
	Если Значения.Получить("dailyStats") <> Неопределено Тогда
		dailyStats = Значения.Получить("dailyStats");
	КонецЕсли;
КонецПроцедуры
...
Функция Данные() Экспорт
	...
	КонецЕсли;
	Если dailyStats <> Неопределено Тогда
		Результат.Вставить("dailyStats", ЗначениеДляJSON(dailyStats));
	КонецЕсли;
	Возврат Результат;
