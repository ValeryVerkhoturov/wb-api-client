// PromotionGetV1PromotionCountResponse200AdvertsInnerAdvertListInner
//
...
// changeTime - Дата - Дата и время последнего изменения кампании
Перем changeTime Экспорт;
...
Процедура Заполнить(Знач Данные) Экспорт
	...
	КонецЕсли;
	Если Значения.Получить("changeTime") <> Неопределено Тогда
		changeTime = Значения.Получить("changeTime");
	КонецЕсли;
КонецПроцедуры
...
Функция Данные() Экспорт
	...
	КонецЕсли;
	Если changeTime <> Неопределено Тогда
		Результат.Вставить("changeTime", ЗначениеДляJSON(changeTime));
	КонецЕсли;
	Возврат Результат;
