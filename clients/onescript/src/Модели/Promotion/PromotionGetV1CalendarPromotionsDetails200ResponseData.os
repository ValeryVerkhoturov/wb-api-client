// PromotionGetV1CalendarPromotionsDetails200ResponseData
//
// Данные ответа
//
...
// promotions - Массив - Список акций
Перем promotions Экспорт;
...
Процедура Заполнить(Знач Данные) Экспорт
	...
	КонецЦикла;
	Если Значения.Получить("promotions") <> Неопределено Тогда
		promotions = Значения.Получить("promotions");
	КонецЕсли;
КонецПроцедуры
...
Функция Данные() Экспорт
	...
	Если promotions <> Неопределено Тогда
		Результат.Вставить("promotions", ЗначениеДляJSON(promotions));
	КонецЕсли;
	Возврат Результат;
