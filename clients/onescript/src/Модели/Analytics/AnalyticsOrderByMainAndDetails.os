// AnalyticsOrderByMainAndDetails
//
...
// field - Строка - Поле для сортировки: - `avgPosition` — по средней позиции - `addToCart` — по добавлениям в корзину - `openCard` — по открытию карточки (переход на страницу това…
Перем field Экспорт;

// mode - Строка - Порядок сортировки: - `asc` — по возрастанию - `desc` — по убыванию
Перем mode Экспорт;
...
Процедура Заполнить(Знач Данные) Экспорт
	...
	КонецЦикла;
	Если Значения.Получить("field") <> Неопределено Тогда
		field = Значения.Получить("field");
	КонецЕсли;
	Если Значения.Получить("mode") <> Неопределено Тогда
		mode = Значения.Получить("mode");
	КонецЕсли;
КонецПроцедуры
...
Функция Данные() Экспорт
	...
	Если field <> Неопределено Тогда
		Результат.Вставить("field", ЗначениеДляJSON(field));
	КонецЕсли;
	Если mode <> Неопределено Тогда
		Результат.Вставить("mode", ЗначениеДляJSON(mode));
	КонецЕсли;
	Возврат Результат;
