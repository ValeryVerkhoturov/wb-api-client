// GeneralGetUsersResponseUsersInnerAccessInner
//
// Класс сформирован автоматически (onescript) из спецификации OpenAPI.
// Правки будут потеряны при следующей генерации.

// code - Строка - Код раздела профиля продавца, к которому пользователь получит доступ: * `balance` — Просмотр баланса и вывод средств * `brands` — Управление брендами * `changeJ…
Перем code Экспорт;

// disabled - Булево - * `true` — доступ к разделу запрещён * `false` — доступ к разделу разрешён
Перем disabled Экспорт;
...
Процедура Заполнить(Знач Данные) Экспорт
	...
	КонецЕсли;
	Если Значения.Получить("disabled") <> Неопределено Тогда
		disabled = Значения.Получить("disabled");
	КонецЕсли;
КонецПроцедуры
...
Функция Данные() Экспорт
	...
	КонецЕсли;
	Если disabled <> Неопределено Тогда
		Результат.Вставить("disabled", ЗначениеДляJSON(disabled));
	КонецЕсли;
	Возврат Результат;
