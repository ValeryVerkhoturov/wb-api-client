// OrdersDbwApiOrdersMetaDleteRequestV2
//
// Класс сформирован автоматически (onescript) из спецификации OpenAPI.
// Правки будут потеряны при следующей генерации.

// key - Строка - Название идентификатора маркировки для удаления. Передаётся только одно значение
Перем key Экспорт;

// ordersIds - Массив - Список ID сборочных заданий
Перем ordersIds Экспорт;
...
Процедура Заполнить(Знач Данные) Экспорт
	...
	КонецЕсли;
	Если Значения.Получить("ordersIds") <> Неопределено Тогда
		ordersIds = Значения.Получить("ordersIds");
	КонецЕсли;
КонецПроцедуры
...
Функция Данные() Экспорт
	...
	КонецЕсли;
	Если ordersIds <> Неопределено Тогда
		Результат.Вставить("ordersIds", ЗначениеДляJSON(ordersIds));
	КонецЕсли;
	Возврат Результат;
