// InStorePickupApiCheckedIdentity
//
// Класс сформирован автоматически (onescript) из спецификации OpenAPI.
// Правки будут потеряны при следующей генерации.

// ok - Булево - Принадлежит ли заказ покупателю: - `true` — принадлежит - `false` — значение не применяется. Если заказ не принадлежит покупателю, вы получите ответ со статус-к…
Перем ok Экспорт;
...
Процедура Заполнить(Знач Данные) Экспорт
	...
	КонецЦикла;
	Если Значения.Получить("ok") <> Неопределено Тогда
		ok = Значения.Получить("ok");
	КонецЕсли;
КонецПроцедуры
...
Функция Данные() Экспорт
	...
	Если ok <> Неопределено Тогда
		Результат.Вставить("ok", ЗначениеДляJSON(ok));
	КонецЕсли;
	Возврат Результат;
