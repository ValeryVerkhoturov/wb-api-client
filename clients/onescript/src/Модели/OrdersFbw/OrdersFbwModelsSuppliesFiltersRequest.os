// OrdersFbwModelsSuppliesFiltersRequest
//
// Класс сформирован автоматически (onescript) из спецификации OpenAPI.
// Правки будут потеряны при следующей генерации.

// dates - Массив - Фильтр по датам
Перем dates Экспорт;

// statusIDs - Массив - Фильтр поставок по статусам. Возможные значения: - `1` — Не запланировано - `2` — Запланировано - `3` — Отгрузка разрешена - `4` — Идёт приёмка - `5` — Принято…
Перем statusIDs Экспорт;
...
Процедура Заполнить(Знач Данные) Экспорт
	...
	КонецЕсли;
	Если Значения.Получить("statusIDs") <> Неопределено Тогда
		statusIDs = Значения.Получить("statusIDs");
	КонецЕсли;
КонецПроцедуры
...
Функция Данные() Экспорт
	...
	КонецЕсли;
	Если statusIDs <> Неопределено Тогда
		Результат.Вставить("statusIDs", ЗначениеДляJSON(statusIDs));
	КонецЕсли;
	Возврат Результат;
