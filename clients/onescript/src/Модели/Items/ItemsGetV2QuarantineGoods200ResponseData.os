// ItemsGetV2QuarantineGoods200ResponseData
//
// Данные ответа
//
...
// quarantineGoods - Массив - Информация о товарах в карантине
Перем quarantineGoods Экспорт;
...
Процедура Заполнить(Знач Данные) Экспорт
	...
	КонецЦикла;
	Если Значения.Получить("quarantineGoods") <> Неопределено Тогда
		quarantineGoods = Значения.Получить("quarantineGoods");
	КонецЕсли;
КонецПроцедуры
...
Функция Данные() Экспорт
	...
	Если quarantineGoods <> Неопределено Тогда
		Результат.Вставить("quarantineGoods", ЗначениеДляJSON(quarantineGoods));
	КонецЕсли;
	Возврат Результат;
