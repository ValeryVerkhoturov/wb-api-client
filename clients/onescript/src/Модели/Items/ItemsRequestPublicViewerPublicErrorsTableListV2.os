// ItemsRequestPublicViewerPublicErrorsTableListV2
//
// Класс сформирован автоматически (onescript) из спецификации OpenAPI.
// Правки будут потеряны при следующей генерации.

// cursor - swagger.PublicErrorsCursorInput
Перем cursor Экспорт;

// order - swagger.PublicErrorsOrderV2
Перем order Экспорт;
...
Процедура Заполнить(Знач Данные) Экспорт
	...
	КонецЕсли;
	Если Значения.Получить("order") <> Неопределено Тогда
		order = Значения.Получить("order");
	КонецЕсли;
КонецПроцедуры
...
Функция Данные() Экспорт
	...
	Если order <> Неопределено Тогда
		Результат.Вставить("order", ЗначениеДляJSON(order));
	КонецЕсли;
	Возврат Результат;
