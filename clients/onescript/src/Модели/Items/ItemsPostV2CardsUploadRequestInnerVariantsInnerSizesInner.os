// ItemsPostV2CardsUploadRequestInnerVariantsInnerSizesInner
//
...
// skus - Массив - Массив баркодов для размера. Если не указать, сгенерируется автоматически. Позволяет связать несколько баркодов с одним размером, например, для разных партий то…
Перем skus Экспорт;
