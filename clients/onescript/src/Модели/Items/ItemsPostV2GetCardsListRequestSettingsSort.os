// ItemsPostV2GetCardsListRequestSettingsSort
//
// Параметр сортировки
//
...
// ascending - Булево - Сортировать по полю `updatedAt`: - `false` — по убыванию - `true` — по возрастанию
Перем ascending Экспорт;
...
Процедура Заполнить(Знач Данные) Экспорт
	...
	КонецЦикла;
	Если Значения.Получить("ascending") <> Неопределено Тогда
		ascending = Значения.Получить("ascending");
	КонецЕсли;
КонецПроцедуры
...
Функция Данные() Экспорт
	...
	Если ascending <> Неопределено Тогда
		Результат.Вставить("ascending", ЗначениеДляJSON(ascending));
	КонецЕсли;
	Возврат Результат;
