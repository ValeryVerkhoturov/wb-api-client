// ItemsBrandsResponseBrandsInner
//
...
// logoUrl - Строка - URL логотипа бренда
Перем logoUrl Экспорт;
...
Процедура Заполнить(Знач Данные) Экспорт
	...
	КонецЕсли;
	Если Значения.Получить("logoUrl") <> Неопределено Тогда
		logoUrl = Значения.Получить("logoUrl");
	КонецЕсли;
...
Функция Данные() Экспорт
	...
	КонецЕсли;
	Если logoUrl <> Неопределено Тогда
		Результат.Вставить("logoUrl", ЗначениеДляJSON(logoUrl));
	КонецЕсли;
