// ItemsGetV2ListGoodsFilter200ResponseData
//
// Данные ответа
//
...
// listGoods - Массив - Информация о товарах
Перем listGoods Экспорт;
...
Процедура Заполнить(Знач Данные) Экспорт
	...
	КонецЦикла;
	Если Значения.Получить("listGoods") <> Неопределено Тогда
		listGoods = Значения.Получить("listGoods");
	КонецЕсли;
КонецПроцедуры
...
Функция Данные() Экспорт
	...
	Если listGoods <> Неопределено Тогда
		Результат.Вставить("listGoods", ЗначениеДляJSON(listGoods));
	КонецЕсли;
	Возврат Результат;
