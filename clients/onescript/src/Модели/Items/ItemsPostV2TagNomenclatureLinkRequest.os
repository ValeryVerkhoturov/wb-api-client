// ItemsPostV2TagNomenclatureLinkRequest
//
...
// tagsIDs - Массив - Массив числовых ID ярлыков. Что бы снять ярлыки с карточки товара, необходимо передать пустой массив. Чтобы добавить ярлыки к уже имеющимся в карточке товара, н…
Перем tagsIDs Экспорт;
...
Процедура Заполнить(Знач Данные) Экспорт
	...
	КонецЕсли;
	Если Значения.Получить("tagsIDs") <> Неопределено Тогда
		tagsIDs = Значения.Получить("tagsIDs");
	КонецЕсли;
КонецПроцедуры
...
Функция Данные() Экспорт
	...
	КонецЕсли;
	Если tagsIDs <> Неопределено Тогда
		Результат.Вставить("tagsIDs", ЗначениеДляJSON(tagsIDs));
	КонецЕсли;
	Возврат Результат;
