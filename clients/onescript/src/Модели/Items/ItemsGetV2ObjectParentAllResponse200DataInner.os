// ItemsGetV2ObjectParentAllResponse200DataInner
//
...
// id - Число - ID родительской категории
Перем id Экспорт;

// isVisible - Булево - Виден на сайте
Перем isVisible Экспорт;
...
Процедура Заполнить(Знач Данные) Экспорт
	...
	КонецЕсли;
	Если Значения.Получить("isVisible") <> Неопределено Тогда
		isVisible = Значения.Получить("isVisible");
	КонецЕсли;
КонецПроцедуры
...
Функция Данные() Экспорт
	...
	КонецЕсли;
	Если isVisible <> Неопределено Тогда
		Результат.Вставить("isVisible", ЗначениеДляJSON(isVisible));
	КонецЕсли;
	Возврат Результат;
